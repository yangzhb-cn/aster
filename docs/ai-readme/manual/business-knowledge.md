...
| 多 Agent 聊天室 | 未实现 | 已实现 | 未实现 | Room、Room Agent CRUD、`@Agent` 触发目前只在 Web 实现。 |
| 归档中心 | 未实现 | 已实现 | 未实现 | 已归档 session、todo、room、room-agent 的恢复和物理删除目前只在 Web 实现。 |
...
- Web Archive 视图集中展示已归档的 session、todo、room、room-agent，并提供恢复和物理删除。
