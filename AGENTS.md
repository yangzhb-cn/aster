...
- 主 runtime 使用 `ContextWindowCache` 维护“旧对话摘要 + 最近完整 turn”；启动时优先从 `workspace/context-windows/` 恢复快照，再用 JSONL 的 seq/hash 增量补齐，快照失效才全量重建。
- 上下文摘要优先走 `LlmSummarizer` 的流式 LLM 语义摘要，失败再回退 `TranscriptSummarizer`；摘要和快照都是运行态缓存，不写入 session 审计历史。
